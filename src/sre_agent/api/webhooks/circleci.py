...
Receives CircleCI webhook events for workflows and jobs.
"""
...
async def circleci_webhook(
    ...
    """CircleCI webhook handler for workflow and job events."""
    raw_body = await request.body()
    headers = dict(request.headers)

    try:
        provider = ProviderRegistry.get_provider(ProviderType.CIRCLECI)
    except Exception as e:
        logger.error(f"Failed to get CircleCI provider: {e}")
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="CircleCI provider not available",
        )

    verification = provider.verify_webhook(headers, raw_body)

    if not verification.valid:
        logger.warning(f"CircleCI webhook verification failed: {verification.error}")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=verification.error or "Invalid webhook signature",
        )
    ...
    delivery_id = compute_fallback_delivery_id(payload, provider="circleci")
    ...
    if isinstance(project, dict):
        repo_hint = project.get("slug") or project.get("name")
    ...
    if not is_new_delivery:
        inc(
            "webhook_deduped",
            attributes={"provider": "circleci", "repo": str(repo_hint or "unknown")},
        )
    ...
    if event_type not in ("workflow-completed", "job-completed"):
        return WebhookResponse(
            status="ignored",
    ...
    with start_span(
        "store_event",
        attributes={"delivery_id": delivery_id, "event_type": "circleci"},
    ):
